...
public sealed class MouseHook : IDisposable
{
    ...
    // Cached at Install() — avoids repeated user32 calls on every hook notification.
    private int _cachedCxDrag;
    private int _cachedCyDrag;
    private uint _cachedDoubleClickMs;
    private int _cachedHalfCxDoubleClk;
    private int _cachedHalfCyDoubleClk;
    ...
    public void Install()
    {
        ...
        _cachedCxDrag = NativeMethods.GetSystemMetrics(NativeMethods.SM_CXDRAG);
        _cachedCyDrag = NativeMethods.GetSystemMetrics(NativeMethods.SM_CYDRAG);
        _cachedDoubleClickMs = NativeMethods.GetDoubleClickTime();
        _cachedHalfCxDoubleClk = NativeMethods.GetSystemMetrics(NativeMethods.SM_CXDOUBLECLK) / 2;
        _cachedHalfCyDoubleClk = NativeMethods.GetSystemMetrics(NativeMethods.SM_CYDOUBLECLK) / 2;
        ...
        if (nCode >= 0)
        {
            ...
            if (message == NativeMethods.WM_LBUTTONDOWN)
            {
                ...
                if (RequireDoubleClick)
                {
                    ...
                    bool withinTime = (now - _lastClickTick) <= _cachedDoubleClickMs;
                    bool withinDistance = Math.Abs(clickPoint.x - _lastClickPoint.x) <= _cachedHalfCxDoubleClk
                                      && Math.Abs(clickPoint.y - _lastClickPoint.y) <= _cachedHalfCyDoubleClk;
    ...
    private bool HasExceededDragThreshold(NativeMethods.POINT from, NativeMethods.POINT to)
    {
        return Math.Abs(to.x - from.x) > _cachedCxDrag
            || Math.Abs(to.y - from.y) > _cachedCyDrag;
    }
