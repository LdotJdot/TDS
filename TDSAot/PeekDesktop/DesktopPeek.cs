...
public sealed class DesktopPeek : IDisposable
{
    ...
    public DesktopPeek(PeekDesktopSettings settings)
    {
        _focusWatcher = new FocusWatcher(PeekDesktopHost.ScheduleForegroundCoalesced);
    ...
    /// <summary>Runs foreground handling on the peek STA / message-loop thread (after coalescing).</summary>
    internal void DeliverForegroundChanged(IntPtr foregroundWindow)
    {
        if (_pauseWhileFullscreenAppActive || _isPeeking)
            UpdateGamingSuppressionState(foregroundWindow);

        if (!_isPeeking || _isTransitioning)
            return;

        AppDiagnostics.LogWindow("Focus changed while peeking", foregroundWindow);
        ...
        if (IsOwnedByCurrentProcess(foregroundWindow))
        {
            AppDiagnostics.Log("Foreground belongs to TDS process; ignoring");
            return;
        }
